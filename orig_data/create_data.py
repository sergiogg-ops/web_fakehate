...
df_fh.loc[df_fh['task'] == 'fake news','label'] = df_fh.loc[df_fh['task'] == 'fake news','label'].apply(lambda x: LABEL['fake news'].index(x))
df_fh['label'] = df_fh['label'].apply(lambda x: [int(x)])
df_fh.loc[df_fh['task'] == 'fake news','test'] = [['fake news detection']]*sum(df_fh['task'] == 'fake news')
df_fh.loc[df_fh['task'] == 'hate speech','test'] = [['hate speech detection']]*sum(df_fh['task'] == 'hate speech')
...
trans = {
    'No sexista': [LABEL['sexism'][1]],
    'Sexista directo NO ESTERIOTIPOS': [LABEL['sexism'][0], LABEL['stereotype'][1]],
    'Sexista directo ESTERIOTIPOS': [LABEL['sexism'][0], LABEL['stereotype'][0]],
}
...
df_exists_memes.rename(columns={'img':'media'},inplace=True)
df_exists_memes['media'] = df_exists_memes['media'].apply(lambda x: 'static/memes_images/'+x)
...
df_exists_tiktoks['media'] = df_exists_tiktoks['id'].apply(lambda x: 'static/tiktok_videos/'+str(x)+'.mp4')
